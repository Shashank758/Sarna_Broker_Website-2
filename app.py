...
# ─────────────────── BUYER DEDICATED PAGES ───────────────────

def _fetch_buyer_bookings(buyer_id):
    """Shared helper: fetch all buyer bookings with miller info."""
    con = get_db()
    ...
    cur.execute("""
        SELECT
            mb.id,
            ms.crop,
            mb.quantity   AS booked,
            mb.loaded_qty AS loaded,
            (mb.quantity - COALESCE(mb.loaded_qty, 0)) AS remaining,
            mb.truck_status,
            mb.loaded_at,
            mb.bill_document,
            mb.loading_status,
            ...
            p.invoice_file              AS final_invoice,
            p.paid_at                   AS payment_at,
            ...
            mb.closed_by,
            mb.price,
            mp.city  AS miller_city,
            mp.state AS miller_state
        FROM miller_bookings mb
        JOIN miller_stock ms ON mb.stock_id = ms.id
        JOIN users u ON ms.miller_id = u.id
        LEFT JOIN payments p ON p.booking_id = mb.id
    ...
    # Fetch loading invoices for all bookings
    booking_ids = [b["id"] for b in bookings]
    invoices_map = {}
    if booking_ids:
        placeholders = ",".join(["%s"] * len(booking_ids))
        cur.execute(f"""
            SELECT id, booking_id, loaded_qty AS qty, invoice_file AS file,
                   truck_number, created_at AS date,
                   qc_weight, qc_moisture, qc_remarks, qc_status, qc_at,
                   final_invoice_file, payment_status, payment_at, qc_freight
            FROM loading_invoices
            WHERE booking_id IN ({placeholders})
        ...
        for r in cur.fetchall():
            invoices_map.setdefault(r["booking_id"], []).append(r)

    # Attach invoices to each booking
    for b in bookings:
        b["invoices"] = invoices_map.get(b["id"], [])
...
def buyer_requested():
    if session.get("role") != "buyer":
        return redirect("/market")
    bookings = _fetch_buyer_bookings(session["user_id"])
    orders = [b for b in bookings if b["status"] == "pending"]
    return render_template("buyer_requested.html", orders=orders)
...
def buyer_active():
    if session.get("role") != "buyer":
        return redirect("/market")
    bookings = _fetch_buyer_bookings(session["user_id"])
    orders = [b for b in bookings
              if b["status"] == "approved"
              and b["loading_status"] in (None, "pending", "partial", "active")]
    return render_template("buyer_active.html", orders=orders)
...
def buyer_loaded():
    ...
    bookings = _fetch_buyer_bookings(session["user_id"])
    orders = [b for b in bookings
              if b["loading_status"] in ("loaded", "partial_closed")]
    return render_template("buyer_loaded.html", orders=orders)
...
def buyer_rejected():
    ...
    return render_template("buyer_rejected.html", orders=orders)


@app.route("/buyer/debit_notes")
def buyer_debit_notes():
    if session.get("role") != "buyer":
        return redirect("/market")
    bookings = _fetch_buyer_bookings(session["user_id"])
    invoices = []
    for b in bookings:
        for inv in b.get("invoices", []):
            if inv.get("final_invoice_file") and inv.get("payment_status") != "paid":
                inv["order_id"] = b["order_id"]
                inv["miller_name"] = b["miller_name"]
                inv["crop"] = b["crop"]
                inv["status"] = inv.get("payment_status", "pending")
                inv["amount"] = ""
                invoices.append(inv)
...
def buyer_payments():
    ...
    cur.execute("""
        SELECT
            ...
            p.invoice_file,     -- 5
            p.paid_at,          -- 6
            COALESCE(mp.mill_name, u.name), -- 7
            mb.loading_status,  -- 8
            mp.city             -- 9
        FROM payments p
        JOIN miller_bookings mb ON p.booking_id = mb.id
        ...
        JOIN users u ON ms.miller_id = u.id
        LEFT JOIN miller_profiles mp ON u.id = mp.miller_id
        WHERE mb.buyer_id = %s AND p.status = 'paid'
...
@app.route("/buyer/profile", methods=["GET", "POST"])
def buyer_profile_page():
    if session.get("role") != "buyer":
        return redirect("/market")
    # Reuse existing profile route if available
